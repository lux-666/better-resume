...
候选人表单包含姓名、技能和至少一个项目（名称、经历）；Job 整体可选，提供时包含岗位、介绍、职责、要求。上传文件在浏览器提取，支持文本、文本型 PDF 与图片 OCR；原始文件不上传。简历在客户端预填，JD 提取文本经 `/api/intake/job` 的一次 Report 模型调用回填四项，用户校对后提交。复杂版式的多项目自动分段尚无准确率基线。
