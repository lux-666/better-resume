...
**状态：1.5-A / 1.5-B v0.1 已实现；真实模型基线与人工 Rubric 待执行**
...
| [1.5-C Model Routing](task-1.5-c-model-routing.md) | Planned | 1.5-B | 离线模型矩阵、路由 Go/No-Go 结论及可回滚 fallback |
...
- v0.1 不使用未经校准的模型自报 confidence 直接路由；
- 1.5-C 的合法结果可以是“不启用动态路由”。没有质量不回退的证据，就保持单模型。
