...
- 成功 Answer 的服务端端到端 p95 ≤25s，包含检索、记忆、模型、校验与提交；同时报告失败率、超时率和浏览器等待时间，附窗口、样本数；
- 并发目标：单进程 20 个活动 Session 无租约冲突、无 SQLite busy 超时；
- 新增 `GET /api/metrics`：按小时聚合的轮次数、p50/p95 延迟、Provider 错误率、降级率、校验拒绝率；
- 技术视图读取该接口展示趋势；复用 Phase 3 统计函数，增加活跃执行数、超时率、备用模型采用率，按 operation/model/status 分组；
- 进度、Trace、metrics 均纳入账户归属与保留策略；聚合缓存可由遥测重建。
