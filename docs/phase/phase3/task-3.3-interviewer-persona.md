...
只改变三处：Interview Agent prompt 的人设与文本规范、`ask_candidate` 的输出结构、页面上非问题类文本。Report Agent、Core 状态模型、完成校验不变。
...
`ask_candidate` 的 question 字段在 Agent 端仍一次性校验，页面侧对已接受的问题做逐字显示，不改变 API 时序。真正的 token 级流式输出需要把校验后置，3.3 不做，记录为 Phase 4 候选。
