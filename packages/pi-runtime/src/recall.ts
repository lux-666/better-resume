...
export interface SessionRecall {
  recall(state: InterviewState, query: RecallQuery, context: { telemetry?: TelemetryCollector; signal?: AbortSignal }): Promise<RecallHit[]>;
}
export function recallTool(options: { memory: SessionRecall; state: InterviewState; telemetry?: TelemetryCollector; signal?: AbortSignal; budget: { remaining: number }; canRead?: () => boolean; onHits?: (hits: RecallHit[]) => unknown }): AgentTool {
  return { name: "recall", label: "Recall session sources", description: "Retrieve earlier turns or accepted evidence within this session, including other projects. OMIT projectId for cross-project corrections; projectId is a strict filter, not the active project context. Resume scope returns unverified source material, never evidence. At most twice per agent per turn.", parameters: RecallQuerySchema,
    execute: async (_, value) => {
      ...
      try {
        const hits = await options.memory.recall(options.state, value, { telemetry: options.telemetry, signal: options.signal });
